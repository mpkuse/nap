...
def worker_score_computation( process_flags, Qd, S_timestamp, S_netvlad, Q_time_netvlad_etc, Q_scores_plot ):
    ...
    while process_flags['MAIN_ENDED'] is False:
        ...
        if Q_scores_plot is not None:
            ...
            # cv2.imwrite( '/home/nvidia/Pictures/zzz12_%d.png' %(curr_item_idx), Qp )
        ...
        n_found = (np.array(__QQW) > 0.6).sum()
        n_items_enqueued = 0
        for idx, scr in _QQW:
            if scr < 0.6:
                continue
...
if __name__ == "__main__":
    ...
    Qd = Queue() # This queue will contains loop-closure candidates. Produced by p_gpu. consumed by p_cpu
